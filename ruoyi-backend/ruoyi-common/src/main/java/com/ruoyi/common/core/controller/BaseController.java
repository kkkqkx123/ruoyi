...
public class BaseController
{
    ...
    /**
     * 响应请求分页数据（使用 MySQL SHOW TABLE STATUS 估算总行数）
     * 适用于百万级大表的分页场景，避免 COUNT(*) 全表扫描
     * 当估算失败时自动回退到 PageInfo 精确统计
     *
     * @param list      当前页数据
     * @param tableName 表名（用于 SHOW TABLE STATUS 估算）
     * @return 分页数据
     */
    @SuppressWarnings({ "rawtypes", "unchecked" })
    ...
    /**
     * 使用 SHOW TABLE STATUS 估算表行数
     * 避免对大表执行精确 COUNT(*)
     *
     * @param tableName 表名
     * @return 估算行数，失败返回 null
     */
    private Long estimateTableRows(String tableName)
    {
        ...
        {
            logger.warn("估算表行数失败 tableName={}", tableName, e);
        }
        return null;
    }
